...
export type EntryFormState   = { errors?: EntryFormErrors; message?: string } | undefined
...
export async function getEntriesByCollection(collectionSlug: string): Promise<{
  ...
  try { await requireCollectionRole(collection.id, session.userId, 'viewer') }
  catch { return null }
  ...
  try {
    ...
    const tagRows = entryIds.length > 0
      ? await db
          .select({ entryId: entryTags.entryId, id: tags.id, name: tags.name, slug: tags.slug })
          .from(entryTags)
          .innerJoin(tags, eq(entryTags.tagId, tags.id))
          .where(inArray(entryTags.entryId, entryIds))
      : []

    const tagsByEntry = tagRows.reduce<Record<string, Pick<Tag, 'id' | 'name' | 'slug'>[]>>(
      (acc, t) => { if (!acc[t.entryId]) acc[t.entryId] = []; acc[t.entryId].push({ id: t.id, name: t.name, slug: t.slug }); return acc },
      {}
    )
...
export async function getEntryById(entryId: string): Promise<EntryDetail | null> {
  ...
  try { await requireCollectionRole(entry.collectionId, session.userId, 'viewer') }
  catch { return null }
...
export async function getOrCreatePageEntry(collectionId: string): Promise<string> {
  const session = await verifySession()
...
export async function saveEntry(
  ...
  if (!validated.success) {
    ...
    return { errors: { slug: e.slug ?? undefined, general: e.content ?? undefined } }
  }

  const { entryId, slug } = validated.data
  let content: unknown
  try { content = JSON.parse(validated.data.content) }
  catch { return { errors: { general: ['Invalid content format.'] } } }
  ...
  try { await requireCollectionRole(existing.collectionId, session.userId, 'editor') }
  catch { return { errors: { general: ['You do not have permission to edit this entry.'] } } }

  // Slug uniqueness within collection
  const [conflict] = await db
    .select({ id: entries.id })
    .from(entries)
    .where(and(eq(entries.collectionId, existing.collectionId), eq(entries.slug, slug)))
    .limit(1)

  if (conflict && conflict.id !== entryId) {
    return { errors: { slug: [`The slug "${slug}" is already used in this collection.`] } }
  }

  await db
    .update(entries)
    .set({ slug, content: content as Record<string, unknown>, updatedAt: new Date() })
    .where(eq(entries.id, entryId))
...
export async function publishEntry(entryId: string): Promise<void> {
  const session = await verifySession()
  const [entry] = await db.select({ collectionId: entries.collectionId }).from(entries).where(eq(entries.id, entryId)).limit(1)
  if (!entry) return
  await requireCollectionRole(entry.collectionId, session.userId, 'editor')
  await db.update(entries).set({ status: 'published', publishedAt: new Date(), updatedAt: new Date() }).where(eq(entries.id, entryId))
  revalidatePath(`/cms/editor/${entryId}`)
}
...
export async function unpublishEntry(entryId: string): Promise<void> {
  ...
  if (!entry) return
  await requireCollectionRole(entry.collectionId, session.userId, 'editor')
  await db.update(entries).set({ status: 'draft', publishedAt: null, updatedAt: new Date() }).where(eq(entries.id, entryId))
  revalidatePath(`/cms/editor/${entryId}`)
}

// ─── Delete Entry ──────────────────────────────────────────────────────────────

export async function deleteEntry(entryId: string, collectionSlug: string): Promise<void> {
  const session = await verifySession()
  ...
  const role = await requireCollectionRole(entry.collectionId, session.userId, 'editor')
