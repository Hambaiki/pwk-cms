...
export async function createCollection(
  ...
  const [existing] = await db
    ...
    .from(collections)
    .where(
      and(eq(collections.ownerId, session.userId), eq(collections.slug, slug)),
    )
    .limit(1);

  if (existing) {
    return {
      errors: { slug: [`You already have a collection with slug "${slug}".`] },
    };
  }
...
// ─── Dashboard stats ───────────────────────────────────────────────────────────
// Returns aggregate counts across all the user's accessible collections.
...
export async function getDashboardStats(
  ...
  if (collectionIds.length === 0) {
    return { totalEntries: 0, publishedEntries: 0, totalMedia: 0 };
  }

  try {
    const [[{ te }], [{ pe }], [{ tm }]] = await Promise.all([
      db
        .select({ te: count() })
        .from(entries)
        .where(inArray(entries.collectionId, collectionIds)),
      db
        .select({ pe: count() })
        .from(entries)
        .where(
      ...
      db
        .select({ tm: count() })
        .from(media)
        .where(inArray(media.collectionId, collectionIds)),
    ]);

    return {
      totalEntries: Number(te),
      publishedEntries: Number(pe),
      totalMedia: Number(tm),
    };
  } catch {
    return { totalEntries: 0, publishedEntries: 0, totalMedia: 0 };
  }
...
export async function getCollectionStats(
  ...
  try {
    ...
    return {
      totalEntries: Number(te),
      publishedEntries: Number(pe),
      draftEntries: Number(de),
      totalMedia: Number(tm),
      totalMembers: Number(mm),
      totalApiKeys: Number(ak),
    };
  } catch {
    return {
      totalEntries: 0,
