...
function ScopeTag({ scope }: { scope: "public" | "private" }) {
  return scope === "public" ? (
    <span className="font-mono text-xs px-1.5 py-0.5 rounded border bg-[rgba(29,158,117,0.08)] border-[rgba(29,158,117,0.3)] text-[#1D9E75]">
      public
    </span>
  ) : (
    <span className="font-mono text-xs px-1.5 py-0.5 rounded border bg-[rgba(83,74,183,0.1)] border-[rgba(83,74,183,0.3)] text-[#7F77DD]">
      private
    </span>
  );
}
...
function RevokeButton({ id }: { id: string }) {
  ...
  const baseBtnCls =
    "px-2.5 py-1 rounded border font-mono text-sm cursor-pointer";

  if (confirming) {
    return (
      <div className="flex items-center gap-1.5">
        <span className="font-mono text-sm text-cms-text-3">Revoke?</span>
        <button
        ...
        return (
          ...
          >
            <div className="flex items-center gap-2 min-w-0">
              ...
              {isRevoked && (
                <span className="font-mono text-xs px-1.5 py-0.5 rounded border shrink-0 bg-[rgba(224,80,80,0.08)] border-cms-danger-border text-cms-danger">
                  revoked
                </span>
              )}
              {!isRevoked && isExpired && (
                <span className="font-mono text-xs px-1.5 py-0.5 rounded border shrink-0 bg-[rgba(186,117,23,0.08)] border-[rgba(186,117,23,0.2)] text-[#BA7517]">
                  expired
                </span>
              )}
            </div>
